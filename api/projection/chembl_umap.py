...
projected_umap = None
try:
    projected_umap = requests.get('http://api_umap:5000/api/projection/umap/').json()[:25_000]
except Exception:
    logger.exception('Error fetching ChEMBL UMAP')

projected_pca = None
try:
    projected_pca = requests.get('http://api_umap:5000/api/projection/pca/').json()[:25_000]
except Exception:
